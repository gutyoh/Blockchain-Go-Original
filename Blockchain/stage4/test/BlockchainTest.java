...
class Block {
    ...
    static Block parseBlock(String strBlock) throws BlockParseException {
        ...
        if (!lines.get(lines.size()-1).toUpperCase().startsWith("N ")) {
            throw new BlockParseException("The last line of every block " +
                    "should be state what happened to N in the format given.");
        }

        if (lines.get(lines.size()-1).toLowerCase().contains("increase")){
            N++;
        } else if (lines.get(lines.size()-1).toLowerCase().contains("decrease")){
            N--;
            if (N < 0){
                throw new BlockParseException("N was decreased below zero!");
            }
        } else if (!lines.get(lines.size()-1).toLowerCase().contains("same")) {
            throw new BlockParseException("The last line of every block " +
